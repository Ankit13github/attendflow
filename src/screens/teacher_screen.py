...
def teacher_tab_take_attendance():
    ...
    subject_options = {
        f"{s.get('name')} - {s.get('subject_code')}": s.get('subject_id')
        ...
        if st.button('Run Face Analysis', width='stretch', type='secondary', icon=':material/analytics:', disabled=not has_photos):
            ...
            with st.spinner('Deep scanning classroom photos...'):
                ...
                for node in enrolled_students:
                    ...
                    if not isinstance(student_id_val, (int, str)):
                        continue
...
def teacher_tab_manage_subjects():
    ...
    if subjects:
        for sub in subjects:
            ...
            stats = [
                ("🫂", "Students", sub.get('total_students')),
...
def teacher_tab_attendance_records():
    ...
    if not records:
        st.warning("There is no record found!")
        return 
    ...
    for r in records:
        ...
        ts_group = ts.split(".")[0] if isinstance(ts, str) else None
        try:
            formatted_time = ( 
