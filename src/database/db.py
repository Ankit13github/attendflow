...
def teacher_login(username, password):
    ...
    if response.data:
        ...
        if isinstance(hashed_password, str) and check_pass(password, hashed_password):
            return teacher
...
def get_teacher_subjects(teacher_id):
    ...
    for sub in subjects:
        ...
        if isinstance(subject_students,list) and subject_students:
            first_item = subject_students[0]

            if isinstance(first_item, dict):
                sub['total_students'] =  first_item.get('count',0)
            else: 
